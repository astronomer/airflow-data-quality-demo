    ...
    def _get_match(self, check_values, record, tolerance=None) -> bool:
        ...
        if "geq_than" in check_values:
            if tolerance is not None:
                match_boolean = record >= check_values["geq_than"] * (1 - tolerance)
            else:
                match_boolean = record >= check_values["geq_than"]
        ...
        if "leq_than" in check_values:
            if tolerance is not None:
                match_boolean = record <= check_values["leq_than"] * (1 + tolerance) and match_boolean
            else:
                match_boolean = record <= check_values["leq_than"] and match_boolean
