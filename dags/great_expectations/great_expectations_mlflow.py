...
def mlflow_multimodel_register_example():
    ...
    def preprocessing(df: pd.DataFrame):
        ...
        df.drop(columns=['education_num', 'relationship',
                         'functional_weight'], inplace=True)
    ...
    @task
    def run_ge_checkpoint(batch_request: RuntimeBatchRequest):
        base_path = Path(__file__).parents[2]
        data_context_root_dir = os.path.join(
            base_path, "include", "great_expectations"
        )
        checkpoint_config = mlflow_checkpoint_config
        checkpoint = Checkpoint(
            data_context=data_context_root_dir,
            **checkpoint_config
        )
                ...
                with mlflow.start_run(run_name=f'{model_type}_{kwargs["run_id"]}') as run:
                    ...
                    if model_type == 'lgbm':
                        ...
                        clf = LogisticRegression(
                            penalty=best_params['penalty'], C=best_params['C'], solver=best_params['solver']).fit(X_train, y_train)
    ...
    def build_best_model(model_params: dict, features: pd.DataFrame, **kwargs):
        ...
        with mlflow.start_run(run_name=f'{model_params["model_type"]}_{kwargs["run_id"]}_best') as run:
            ...
            if model_params['model_type'] == 'lgbm':
                ...
                base_params = {'objective': 'binary', 'metric': [
                    'auc', 'binary_logloss'], 'boosting_type': 'gbdt'}
    ...
    def register_model(model_run_id: str):
        ...
        mv = mlflow.register_model(
            f'runs:/{model_run_id}/model', 'census_pred',)
